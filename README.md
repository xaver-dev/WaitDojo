...
**Turn AI waiting time into training time.**

WaitDojo is a tiny Windows tray app: whenever a Claude Code tool call runs longer than 60 seconds, a small always-on-top popup appears in the corner with 5 quiz cards. Answer them while you wait, close it, get back to work.
...
The hook script is fail-safe: if WaitDojo isn't running it exits silently in ~150 ms and never blocks Claude Code.
...
WaitDojo is free and open source. To keep the door open for an optional premium version later (if this project takes off), the free version has two limits from day one — no surprises down the road:
...
4. Restart WaitDojo. With multiple decks, switch via tray menu → "Switch deck".
...
Invalid deck files produce a clear error dialog on startup instead of a crash. Learning progress is stored per deck (`%APPDATA%\waitdojo\progress-<deck-id>.json`), so switching decks keeps every deck's progress.
...
Only active when started with env `WAITDOJO_DEBUG=1`: `GET /debug/quiz`, `POST /debug/fill {"answers":[…]}`, `POST /debug/finish`, `POST /shot {"path":"out.png"}` (popup screenshot). Off in normal operation.
